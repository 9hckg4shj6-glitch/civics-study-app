/* ============================================================
   ...
   科目ごとの表示設定（無いときはアプリ側の既定＝公共政経の並びを使う）:
     ...
     draft            … true の間は科目えらびに「準備中」で出し、タップしても中へ入れない。
                        ...
                        いまはどの科目にも付いていない
     app              … 中の画面をクイズ画面と共用せず、別アプリとして描く科目（いまは "kanbun" だけ）。
                        タイルを押すと index.html の enterMiniApp が window.STUDY_CORE[app] を開く。
                        contentDir / questions は持たない
     expectItems      … app 科目のタイルに出す「◯項目」の既定値（学習記録がまだ無いときに使う）
...
window.SUBJECTS = [
  ...
  {
    /* 漢文。科目えらびのタイルだけを共有し、中の画面は src/kanbun/ の別アプリ（app:"kanbun"）が描く。
       クイズ画面を使わないので contentDir / questions は持たせない（build:questions・validate:content の対象外）。
       教材は content/kanbun/*.json、検査は tests/kanbun-*.test.ts。方針は docs/kanbun/設計.md が正本。 */
    id: "kanbun",
    app: "kanbun",
    name: "漢文",
    emoji: "📜",
    accent: "#b7282e",       // 朱
    idPrefix: "kb-",
    expectItems: 500,         // 学習項目（一の門「語」の語150＋二の門「字」の用法173＋三の門「句」の句形107＋四の門「試」の漢詩30＋書棚の常識40）の数。教材を増やしたらこの数も更新する
  },
];
