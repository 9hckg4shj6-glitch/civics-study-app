...
うち55問は図表を伴う。図は `public/images/civics/*.webp` に置き、すべてに `imageAlt`（代替テキスト）を付けている。
表やグラフのうち数値を文章で書き表せるものは、画像を作らずに問題文へ転記している。

正解の確定方法は年度で異なる。令和8年度（本試験・追再試験）・令和7年度（本試験・追再試験）・令和6年度（本試験・追再試験）・令和4・3年度は
大学入試センター公表の正解PDF・正解表と照合済み。令和5年度の24問は公式正解表を入手できていないため、出版社の解説冊子と
...
> 公式正解表を入手できていない点は令和5年度と同じ扱いで、`sources/INDEX.md` に記録してある。
