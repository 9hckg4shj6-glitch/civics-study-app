...
| 日本史（共テ対策） | 共通テスト「歴史総合，日本史探究」・センター試験「日本史B」の過去問（準備中） | — |
| 化学（私立対策） | 私立大学の一般入試の化学から、共テ対策と同じ形式の正誤問題（準備中） | [docs/化学科目_実装計画.md](docs/化学科目_実装計画.md)（分類・出典区分を共有） |

「準備中」の2科目は `public/subjects.js` で `draft: true` になっていて、科目えらびのタイルは出るが
...
0問（準備中）。日本史の大分野は時代の5区分（原始・古代／中世／近世／近代／現代）、出典区分は
共通テスト「歴史総合，日本史探究」（新課程）／共通テスト「日本史B」（旧課程）／センター試験「日本史B」／自作。
小分野（`fieldOrder`）の表は最初の収録と一緒に決める。
化学（私立対策）は共テ対策の化学と同じ分類・出典区分・問い方（`askType`）・検査を使い、IDの接頭辞（`chemp-`）と
教材の置き場所だけを分ける。
