...
describe("化学（私立対策）の科目設定", () => {
  ...
  it("準備中ではなく、収録数・分野別の内訳が宣言と合い、私立の出典で年度を持つ", () => {
    ...
    for (const q of privQuestions) {
      ...
      // 「大学別」の演習・一覧はこの欄で束ねる（sourceLabel からの切り出しはしない）。同じ大学は必ず同じ表記
      expect(["自治医科大学", "岩手医科大学"]).toContain(q.university);
      // 見出しには年度ではなく「自治医科大2025年」のような短い出典名（大学名の「学」を落とした略＋年度）を出す
      expect(q.sourceShort).toBe(`${String(q.university).replace(/学$/, "")}${String(q.year).replace("年度", "")}年`);
